...
전국 고속도로·국도 **실시간 소통정보**, **CCTV**, **사고·돌발 알림**을 제공하는 MCP 서버이자 모바일/데스크탑 하이브리드 웹 앱(PWA)입니다.
데이터는 국토교통부 ITS 국가교통정보센터 OpenAPI(교통소통정보·돌발상황정보·CCTV정보)를 사용하고, 지도는 [MapLibre GL JS](https://maplibre.org/)로 그립니다.

## 📋 Overview

| 구성 | 설명 |
|------|------|
| **웹 대시보드** (`data-go-mcp.its-road-traffic-web`) | 전국 지도, 고속도로/국도 노선 목록 → 노선 상세, 노선별 CCTV 목록·영상 재생, 사고·돌발 발생 시 알림 |
| **MCP 서버** (`data-go-mcp.its-road-traffic`) | Claude 등 AI 도구에서 노선별 소통 현황, 돌발 정보, CCTV를 조회 |

### 화면 구성

- **모바일 (< 1024px)**: 하단 탭 `노선 · 전국지도 · 돌발 · CCTV · 알림`
  - **노선**: 고속도로/국도 전환, 노선번호·평균 속도·원활/서행/정체 비율 막대·돌발 수·CCTV 수, ★ 관심 노선
    - 노선 선택 → 상세: 노선 지도, 진행 중 돌발, 노선 CCTV(기점→종점 순), 방향별(상행/하행) 소통과 가장 느린 구간
  - **전국지도**: 전국 노선을 소통 등급 색으로 그리고 돌발·CCTV를 함께 표시. `소통 / 돌발 / CCTV` 레이어 켜고 끄기, CCTV는 줌에 따라 묶음(클러스터)으로 표시
  - **돌발**: 전국 돌발 피드(유형 필터). 항목을 누르면 전국 지도에서 위치를 보여주고, 5km 안에 CCTV가 있으면 `▶ CCTV 1.2km` 버튼으로 바로 영상 확인
  - **CCTV**: `전체/고속도로/국도` + `노선별 목록 | 전국 지도` 전환
    - 노선별 목록: 노선마다 묶어 개수 표시, 펼치면 노선을 따라 정렬된 CCTV, 노선·지점명 검색, `지도` 버튼으로 해당 노선 CCTV만 지도에서 보기
    - 전국 지도: 전국 CCTV를 지도에서 보고 눌러서 재생
  - **알림**: 알림 권한, 푸시, 알림 받을 유형/도로/관심 노선 설정, 최근 알림
- **데스크탑 (≥ 1024px)**: `노선 소통·CCTV 목록 | 전국 지도 | 노선 상세 + 돌발 피드` 3단 화면, 🔔 버튼으로 알림 설정
- **CCTV 플레이어**: 실시간 HLS(Safari 기본 재생, 그 외 hls.js), MP4, 정지영상(5초마다 갱신) 지원. `이전/다음`으로 같은 노선 CCTV를 차례로 보기, `지도에서 위치`
...
2. `교통소통정보`, `돌발상황정보`, `CCTV 화상자료` API 사용 신청 → 인증키 발급
3. 환경변수 `ITS_API_KEY`(또는 `API_KEY`)에 설정

> ⚠️ ITS OpenAPI는 **국내 IP에서만 응답**합니다. 해외 서버/클라우드에서는 연결이 거부되므로 국내 서버·PC에서 실행하세요.
> 개발계정은 **하루 1,000건** 제한이 있어 기본 설정이 이에 맞춰져 있습니다 (돌발 3분 주기 480건 + 소통 10분 캐시 × 고속도로·국도 288건 + CCTV 30분 캐시 × 고속도로·국도 96건 = 최대 864건).
...
- [MapLibre GL JS](https://maplibre.org/) 5.x (CDN, SRI 무결성 검사)로 그립니다.
- 기본 배경 지도는 API 키가 필요 없는 [OpenFreeMap](https://openfreemap.org/)(OSM 기반 벡터 타일)의 `positron`/`dark` 스타일이며, 스타일 서버에 접속할 수 없으면 OSM 래스터 타일로 자동 전환합니다.
- `ITS_MAP_STYLE_URL`로 MapTiler, VWorld 등 원하는 MapLibre 스타일 JSON을 지정할 수 있습니다.

### CCTV 영상 재생

- ITS가 주는 CCTV 영상 URL에는 **만료되는 인증 토큰**이 들어 있어 목록을 30분마다(화면은 15분마다) 새로 받습니다. 그래도 재생이 안 되면 잠시 후 다시 열어 보세요.
- 브라우저가 CCTV 서버에서 직접 영상을 받지 못하는 경우(CORS 미허용, HTTPS 페이지에서 HTTP 영상을 여는 혼합 콘텐츠 차단)를 위해 서버가 영상을 중계하는 프록시(`/api/cctv/proxy`)가 있습니다. 직접 재생에 실패하면 자동으로 프록시로 다시 시도합니다.
  - 프록시는 **CCTV 목록에 나온 호스트만** 중계하므로 임의 주소를 여는 공개 프록시로 쓰일 수 없습니다.
  - 영상이 서버를 거치므로 트래픽이 늘어납니다. 필요 없으면 `ITS_CCTV_PROXY=false`로 끌 수 있습니다.
- `ITS_CCTV_TYPE`으로 영상 형식을 바꿀 수 있습니다 (기본 `4` 실시간 HLS·HTTPS, `1` HLS·HTTP, `2` 동영상, `3` 정지영상, `5` 동영상·HTTPS).
...
| `ITS_CCTV_TTL_SECONDS` | `1800` | CCTV 목록 캐시 유효시간(초). 영상 URL 토큰 만료보다 짧게 |
| `ITS_CCTV_TYPE` | `4` | CCTV 영상 형식 코드 (위 참고) |
| `ITS_CCTV_PROXY` | `true` | CCTV 영상 중계 프록시 사용 여부 |
| `ITS_MAP_STYLE_URL` | OpenFreeMap positron | MapLibre 스타일 JSON URL |
| `ITS_MAP_STYLE_DARK_URL` | OpenFreeMap dark | 다크 모드 스타일 (미지정 시 `ITS_MAP_STYLE_URL`을 따라감) |
...
| GET | `/api/cctv?type=all\|ex\|its&q=` | 노선별로 묶은 CCTV (노선 안에서는 기점→종점 순) |
| GET | `/api/cctv/proxy?u=` | CCTV 영상 중계 (CCTV 목록의 호스트만 허용, HLS 플레이리스트 주소 재작성) |
...
| GET | `/api/demo/cctv/{id}.svg` | (데모 모드 전용) 가상 CCTV 화면 |
...
| `list_cctv(road_name, road_type, limit)` | 노선별로 묶은 CCTV와 영상 URL | `list_cctv("경부")` |

질문 예시: "지금 경부고속도로 상행 막혀?", "전국 고속도로에서 난 사고 알려줘", "영동고속도로 공사 구간 있어?", "판교 근처 CCTV 주소 알려줘"
...
- CCTV는 `[경부선] 양재`처럼 이름 앞 대괄호의 노선명으로 묶습니다. 노선명이 없는 CCTV는 `노선 미분류`로 모입니다.
- CCTV 응답 형식(`response.data`, 소문자 필드명)과 `cctvType` 코드는 공개 자료를 참고해 구현했으므로, 실제 응답이 다르면 `api_client.py`의 파싱을 조정해야 할 수 있습니다.
